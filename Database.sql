...
--Returneaza o lista cu toti angajatii a caror functie contine un anumit cuvant pe care trebuie sa il dam.
CREATE FUNCTION f_Angajati_Functie_Contine(@text VARCHAR(30))
RETURNS TABLE
AS
RETURN
    ...
    WHERE F.Denumire LIKE '%' + @text + '%';
GO

SELECT * FROM f_Angajati_Functie_Contine('inginer'); --Exemplu de folosire a functiei

--Salariile dintr-un departament
CREATE FUNCTION f_Salarii_Departament(@dept VARCHAR(30))
RETURNS TABLE
AS
RETURN
    SELECT F.Salariu
...
--Cati angajati au un anumit salariu
SELECT F.Salariu, COUNT(*) AS NrAngajati
FROM ANGAJATI A
JOIN FUNCTII F ON A.IdFunctie = F.IdFunctie
WHERE F.Salariu IN (SELECT Salariu FROM Departamente WHERE Denumire ='PRODUCTIE')
GROUP BY F.Salariu;

--Pentru a vedea departamentele
select *from DEPARTAMENTE

--Functie pentru afisarea salariului minim si maxim dintr-un departament
CREATE FUNCTION f_MinMax(@dept VARCHAR(30))
RETURNS TABLE
AS
RETURN (
    SELECT 
        MIN(F.Salariu) AS MinSalariu,
        MAX(F.Salariu) AS MaxSalariu
    FROM ANGAJATI A
    JOIN FUNCTII F ON A.IdFunctie = F.IdFunctie
    JOIN DEPARTAMENTE D ON A.IdDept = D.IdDept
    WHERE D.Denumire = @dept
...
SELECT * FROM f_MinMax('MANAGEMENT'); --exemplu de implementare functie

--Functie pentru afisarea numarului de produse vandute intr-o perioada
CREATE FUNCTION f_Produse_Perioada(@d1 DATE, @d2 DATE)
RETURNS TABLE
AS
RETURN
    ...
    WHERE V.DataVanz BETWEEN @d1 AND @d2;
GO

SELECT * FROM f_Produse_Perioada('2016-05-01', '2016-06-08'); --exemplu de implementare

--Suma totala acumulata de un vanzator al carui nume este trimis ca parametru

CREATE FUNCTION f_Suma_Vanzator(@nume VARCHAR(20))
RETURNS INT
AS
BEGIN
    DECLARE @suma INT;
...
SELECT dbo.f_Suma_Vanzator('N8'); --exemplu de implementare


--Functie verificare prag, ne arata cati angajati au depasit pragul de vanzari dat (suma data)
CREATE FUNCTION f_Depaseste_Prag(@nume VARCHAR(20), @prag INT)
RETURNS BIT
AS
BEGIN
    IF dbo.f_Suma_Vanzator(@nume) > @prag
        RETURN 1;
    RETURN 0;
END;
GO

SELECT dbo.f_Depaseste_Prag('N8', 200); --exemplu implementare

--Angajatii care au vandut peste 100 RON

SELECT Nume, Prenume
FROM ANGAJATI
WHERE dbo.f_Depaseste_Prag(Nume, 100) = 1;

--Functie pentru afisare cele mai vandute N produse dintr-o perioada

CREATE FUNCTION f_Produse_Vandute(@d1 DATE, @d2 DATE)
RETURNS TABLE
AS
RETURN
...
--Exemple de implementare a functiilor
SELECT * FROM dbo.f_MinMax('PRODUCTIE');
SELECT * FROM dbo.f_Angajati_Functie_Contine('ING');


--Cream un „tabel virtual” (un VIEW).
...
--Extragem numele si prenumele celor care lucreaza in productie
SELECT Nume, Prenume
FROM vAngajati
...
--Functii pentru afisare media salariala, salariul minim/maxim, suma salariilor
select avg(salariu) from vangajati
select min(salariu) from vangajati
select max(salariu) from vangajati
select sum(salariu) from vangajati

SELECT Departament, Salariu SalMed
FROM vAngajati
GROUP BY Departament, Salariu 
...
select *from vangajati where departament= 'Management'
...
select *from vangajati where Nume LIKE '%1%' --afiseaza toti angajatii care contin cifra '1' in nume
select *from vangajati where Nume LIKE 'N1%' --afiseaza toti angajatii a caror nume incepe cu "N1"
...
SELECT min(Salariu) Salmin
FROM vAngajati
where departament= 'Management'


SELECT max(Salariu) Salmax
FROM vAngajati
where departament= 'Management'

SELECT Departament, min(Salariu) Salmin, max(Salariu) Salmax
FROM vAngajati
...
SELECT Departament, sum(Salariu) Salsum
FROM vAngajati
GROUP BY Departament
ORDER BY Salsum ASC
...
FROM vAngajati
ORDER BY Departament, vechime 


SELECT Functie, Nume, Prenume
FROM vAngajati
WHERE DATEDIFF(year , DataAngajarii, GetDate()) > 10
GROUP BY Functie, Nume, Prenume

---Care sunt angajatii, grupati pe departamente, care au varsta de minim 30 ani? (GROUP BY)
select* from ANGAJATI

SELECT Departament, Nume, Prenume
FROM vAngajati
WHERE DATEDIFF(year , DataNasterii, GetDate()) > 29
GROUP BY Departament, Nume, Prenume

---Daca vrem sa afisam numarul de ang. peste 30 pe dept.

---SELECT Departament, COUNT(*) AS NrAngajati_30plus
---FROM vAngajati
---WHERE DATEDIFF(YEAR, DataNasterii, GETDATE()) > 29
--GROUP BY Departament;
